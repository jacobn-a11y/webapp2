...
export class WorkflowAutomationService {
  ...
  private async sendSlackMessage(
    ...
    try {
      ...
      throw new Error(`Slack send failed: ${err instanceof Error ? err.message : "unknown"}`);
    }
  }
  ...
  private async sendEmail(
    ...
    // Email sending would integrate with SendGrid/SES/etc in production
  ...
  private async sendWebhook(
    ...
    try {
      ...
      throw new Error(`Webhook failed: ${err instanceof Error ? err.message : "unknown"}`);
    }
  }
  ...
  private async getMetricValue(organizationId: string, metric: string): Promise<number | null> {
    // Metric resolution — would query actual metric sources in production
    switch (metric) {
      case "dlq_count": {
