...
export class PolicyEngine {
  ...
  private async checkRoleProfile(
    ...
    // Check if any of the profile's permissions map to this action
  ...
  private async checkAccountScope(
    ...
    // Check role profile default scope
    if (subject.roleProfileId) {
  ...
  private async checkOrgPolicy(
    ...
    // Check feature flags that gate specific actions
    if (action.startsWith("export:")) {
      ...
      if (governance && !governance.allow_named_story_exports) {
        // Check if this export involves named stories
        // This would need resource context in practice
      }
    }
  ...
  private async checkMfaRequirement(
    ...
    // Check if org requires MFA for privileged actions
    try {
